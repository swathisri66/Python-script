...
class TestSmsPrimaryClasses(BaseSmsTestCases):
    ...
    def test_api_response(self, sender):
        """Test API response"""
        status, response = \
            sender.set_recipient(self.CORRECT_PHONE).set_content(self.CORRECT_CONTENT).send()
        assert status
        assert response == {'recipient': '0048123456789', 'status': 'SENT', 'api': '1'}

    def test_recipient_not_set(self, sender):
        """Test exception when phone number is not set"""
    ...
    def test_compare_old_and_new(self, sender):
        ...
        assert (
            sms_primary_api(self.CORRECT_CONTENT, self.CORRECT_PHONE, self.CORRECT_COUNTRY) ==
            sender.set_recipient(self.CORRECT_PHONE).set_content(self.CORRECT_CONTENT).send()
        )
...
class TestSmsSecondaryClasses(BaseSmsTestCases):
    ...
    def test_compare_old_and_new(self, sender):
        """Test should compare old and new functionality implementation"""
        assert (
            sms_secondary_api(self.CORRECT_CONTENT, self.CORRECT_PHONE, self.CORRECT_COUNTRY) ==
            sender.set_recipient(self.CORRECT_PHONE).set_content(self.CORRECT_CONTENT).send()
        )
